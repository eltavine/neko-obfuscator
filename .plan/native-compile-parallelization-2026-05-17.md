...
### [ ] P38: Select next native optimization from accepted post-P35 baseline

- Scope: choose the next generic architecture-level native optimization from accepted post-P35 code, not from rejected P37 artifacts. The next row must be backed by fresh accepted-source evidence and must avoid rejected P27 nested-store, P29 static-setter, P33 string-ref, P34 smaller-helper-shard, P36 method-pointer-ref, and P37 fieldID-elision shapes unless new root-cause evidence explains their failures.
- Required evidence: gather fresh accepted-source generated-C/runtime data after P37 rejection if artifacts were overwritten, then identify an exact invariant/path/site and validation target before implementation.
