...
### [ ] P26: Compress generated primitive-array literal initialization for build speed

- Scope: reduce native build long tails by replacing straight-line generated primitive array literal stores with a generic compact native initialization form. This must apply only to bytecode-proven literal initialization shapes, preserve allocation, null, bounds, and exception behavior, keep JVM ABI and native coverage unchanged, and introduce no JNI/JVMTI/original-bytecode fallback.
- Required evidence: P24/P25 manifests show evaluator remains the largest build artifact with `142` C files and long implementation compile tails; generated evaluator `neko_native_impl_103.c` is about `50KB` and repeatedly emits `neko_fast_new_primitive_array(... NEKO_PRIM_I)` followed by constant-index `neko_checked_iastore(...)` stores. This is a generic bytecode/codegen pattern, not an evaluator-specific path.
- Validation command or runtime target: add/adjust focused translator/codegen coverage for primitive-array literal initialization; run focused native audit tests; freshly regenerate evaluator plus TEST/test21/SnakeGame native-only artifacts; compare evaluator top implementation compile time, compile sum, generated C size, runtime evaluator completion, and four-jar smoke; inspect generated C for no forbidden JNI/JVMTI/fallback markers and unchanged compiler flags.
