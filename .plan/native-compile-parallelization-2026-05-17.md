...
### [ ] P35: Select next native optimization from accepted post-P32 baseline

- Scope: choose the next generic architecture-level native optimization from accepted post-P32 code, not from rejected P33/P34 artifacts. The next row must be backed by fresh accepted-source evidence, must refresh `:neko-cli:installDist` after any source revert before generation, and must avoid rejected P27 nested-store, P29 static-setter, P33 string-ref, and P34 smaller-helper-shard shapes unless new root-cause evidence explains their regressions.
- Required evidence: gather fresh accepted-source generated-C/runtime data after P34 rejection if artifacts were overwritten, then identify an exact invariant/path/site and validation target before implementation.
