...
### [ ] P28: Shorten repeated invoke callsite boilerplate without changing dispatch semantics

- Scope: target the next evidence-backed runtime/build shape candidate: replace repeated generated direct/virtual invoke boilerplate with generic short callsite wrappers or helper forms while preserving existing `neko_icache_dispatch`, NJX/direct-call semantics, receiver null behavior, exception propagation, hidden-key/CFF behavior, compiler flags, and no JNI/JVMTI/original-bytecode fallback policy.
- Required evidence: before implementation, fill this row with current P26C/P27B generated C counts and exact hot methods showing repeated invoke fragments such as `neko_icache_dispatch`, `neko_njx_*`, `neko_bound_method_i_entry`, and bound-class expressions; the evidence must identify a generic callsite shape and prove it affects build size and/or TEST/test21 runtime metrics.
- Validation command or runtime target: before implementation, fill with focused `CCodeGeneratorTest`, `OpcodeTranslatorUnitTest`, `NativeGeneratedCHotPathAuditTest`, fresh TEST/test21/SnakeGame/evaluator native-only generation, direct runtime validation, generated-C forbidden-marker inspection, and timing comparison against immediate before-edit artifacts.
- Completion criteria: before implementation, fill with exact no-regression and improvement targets; no reduced native coverage, compiler-flag weakening, dispatch semantic change, JNI/JVMTI fallback, skip-on-error behavior, or stale-artifact proof is allowed.
