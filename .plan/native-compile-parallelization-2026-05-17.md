...
### [ ] P37: Select next native optimization from accepted post-P35 baseline

- Scope: choose the next generic architecture-level native optimization from accepted post-P35 code, not from rejected P36 artifacts. The next row must be backed by fresh accepted-source evidence and must avoid rejected P27 nested-store, P29 static-setter, P33 string-ref, P34 smaller-helper-shard, and P36 method-pointer-ref shapes unless new root-cause evidence explains their regressions.
- Required evidence: gather fresh accepted-source generated-C/runtime data after P36 rejection if artifacts were overwritten, then identify an exact invariant/path/site and validation target before implementation.
