...
### [x] P9: Compress generated string-concat C without changing native semantics

- Scope: replace repeated translated `neko_concat_accumulate(thread, env, __acc, rhs, g_off_String_value, g_off_String_coder)` argument boilerplate with a generated `neko_concat_accumulate_string(thread, env, acc, rhs)` macro that expands to the existing `neko_concat_accumulate` helper and the same resolved `java/lang/String.value:[B` plus `java/lang/String.coder:B` offset slots. This is C source compaction only: keep the existing native concat normalization, `neko_require_fast_string_concat` path, NJX concat fallback inside the support helper, `zig cc`, `-O3`, `NEKO_FLATTEN`, native method coverage, field-offset binding, runtime helper policy, JNI/JVMTI policy, and original-bytecode fallback policy unchanged.
- Required evidence: accepted P39 artifacts are the current baseline; rejected P40 artifacts must not be used. Fresh P39 generated-C inspection shows concat source concentration is generic and material in test21: `134` `neko_concat_accumulate` callsites across translated implementation files, including `70` in the largest `test21` source `neko_native_impl_39.c` (`124985` bytes). Other accepted P39 artifacts have smaller concat surfaces (TEST `1`, evaluator `5`, SnakeGame `0`), so validation must prove no semantic or performance regression while checking for a test21 compile/source-size improvement. This row explicitly avoids rejected P33 string-literal descriptors, rejected P36 method-pointer compaction, rejected P37 fieldID elision, and rejected P40 current-owner macro shape.
- Validation command or runtime target: focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`; direct runtime validation for TEST/test21/evaluator and timeout-guarded SnakeGame smoke with `-Djava.io.tmpdir=build/native-run-tmp`; TEST/test21 timing repeats with `-XX:+PerfDisableSharedMem`; generated-C inspection showing `neko_concat_accumulate_string` callsites, reduced direct `neko_concat_accumulate(thread, env, __acc, ... g_off_...)` fragments, and unchanged support helper definitions; stdout/stderr, fresh `hs_err`, manifest, and forbidden JNI/JVMTI/fallback/original-bytecode marker inspection.
- Completion criteria: refreshed artifacts report `translated>0 rejected=0`; generated impl C uses the concat macro for translated concat callsites while the macro still expands to the existing helper and String field-offset slots; direct runtime exits cleanly for TEST/test21/evaluator and Snake timeout smoke; TEST Calc and test21 Platform/Virtual/Seq timings do not regress beyond current P39 noise and at least one test21 build-size/compile-tail/wall-time metric improves; no semantic change to concat null normalization, String field-offset resolution, NJX concat fallback, native coverage, compiler flags, JNI/JVMTI fallback, skip-on-error behavior, original-bytecode fallback, or stale-artifact proof is introduced.
- Completion evidence: implemented `neko_concat_accumulate_string(thread, env, acc, rhs)` as a generated macro over the existing `neko_concat_accumulate` helper and the same `java/lang/String.value:[B` / `java/lang/String.coder:B` offset slots. `OpcodeTranslator.appendDirectStringConcat` now explicitly registers both offset slots before emitting the macro, and focused tests include a full translated invokedynamic concat source check for macro declaration/use. Focused Gradle validation passed with `./gradlew :neko-test:test --tests dev.nekoobfuscator.test.OpcodeTranslatorUnitTest --tests dev.nekoobfuscator.test.CCodeGeneratorTest --tests dev.nekoobfuscator.test.NativeGeneratedCHotPathAuditTest` using repository-local `java.io.tmpdir`. Fresh P9 native-only generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. Generated-C inspection showed test21 uses `134` `neko_concat_accumulate_string` translated callsites and `0` old direct translated `neko_concat_accumulate(thread, env, __acc, ...)` fragments; the support helper definition remains present once and the macro expands to `neko_concat_accumulate((thread), (env), (acc), (rhs), g_off_..., g_off_...)`. Direct runtime passed for TEST (`Calc: 38ms`, `ReTrace PASS` retained), test21 (`Platform/Virtual/Seq 45/39/28ms`), evaluator, and timeout-guarded SnakeGame smoke with no new `hs_err`. A 5x timing repeat group showed no accepted-regression signal against current P39 noise: medians were Calc `41ms`, Platform `43ms`, Virtual `41ms`, Seq `29ms`, Parallel/VThreads `2ms`. The targeted test21 largest source improved from P39 `124985` to `124215` bytes and top compile tail improved `2101ms -> 1996ms`; compile-sum noise was worse `42709ms -> 43548ms`, so acceptance is based on the targeted source-size/top-tail improvement and clean runtime. Forbidden-marker scan over fresh P9 artifacts found `0` `NEKO_JNI_FN_PTR`, `(*env)->`, `env->`, direct `Call*Method`, `Get*Field`, `FindClass`, `NewStringUTF`, `NewObject*`, `Throw*`, JVMTI, `translated=0`, `Native compilation produced no libraries`, original-bytecode, or skip-on-error markers in generated impl sources. Initial read-only review found a missing-offset registration risk; the fix added explicit offset registration and a full-source test, and re-review found no remaining scoped issues.
...
### [x] P41: Strip release native shared libraries at link time

- Scope: add release-only shared-library stripping to the native link command so packaged `libneko_*` artifacts do not retain DWARF/debug/symtab sections. Preserve `NEKO_NATIVE_DEBUG=1` behavior with `-g`/frame pointers and unstripped symbols for crash investigation; preserve all generated C, native coverage, compiler optimization flags, method selection, runtime helper behavior, dynamic symbol requirements, JNI/JVMTI policy, and fallback policy. This is a generic release packaging/link step, not a jar-specific size workaround.
- Required evidence: full `./gradlew test` after P9/P4 failed `NativeObfuscationPerfTest.nativeObfuscation_TEST_sharedLibrarySizeWithinSanityBounds` because the TEST native library was `6,879,440` bytes, above the `5MB` release sanity bound. Fresh native libraries report `with debug_info, not stripped`; section inspection shows `.debug_loc`, `.debug_abbrev`, `.debug_info`, `.debug_ranges`, `.debug_str`, `.debug_line`, `.debug_frame`, `.symtab`, and `.strtab` in release output. A repository-local relink experiment using the same TEST object files plus `zig cc ... -shared -s` produced a stripped ELF shared object of `1,052,456` bytes while retaining `.dynsym`, proving the size excess is retained debug/symbol metadata rather than translated-code coverage. The change must be link-level and generic, and must not remove required dynamic loader symbols.
- Validation command or runtime target: focused native generated-C/build tests; fresh TEST/test21/SnakeGame/evaluator native-only generation; inspect manifests for release link command containing `-s` and debug builds not using it; inspect packaged TEST library size under the 5MB threshold; run TEST/test21/evaluator native jars plus Snake timeout smoke; rerun the failing `NativeObfuscationPerfTest.nativeObfuscation_TEST_sharedLibrarySizeWithinSanityBounds`; inspect fresh libraries for stripped debug sections, retained dynamic symbols, no fatal runtime markers, and no fallback/JNI/JVMTI markers.
