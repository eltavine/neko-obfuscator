...
Status: `[x]` rejected after measurement; source reverted.
...
2. `[x]` Implement, validate, measure, and either accept or reject the
   ...
   artifact, record rejection evidence, and commit only the rejection record.
   Rejected: the issue-only helper implementation compiled and the broad
   targeted JVM suite passed with `BUILD SUCCESSFUL in 18s` and `19 actionable
   tasks: 19 executed`. Generated bytecode inspection at
   `build/jvm-runtime-perf/p2-2-17-after-test-a-a.javap.txt` showed the
   issue-only TEST helper `a.a.n:(JIII[Ljava/lang/Object;II)J` was `97`
   JIT-reported bytes, had no Thread/StackWalker/runtime-source branch and no
   ticket-mode branch, called token helper `j` twice, and called ticket helper
   `h` once. Routing inspection at
   `build/jvm-runtime-perf/p2-2-17-after-kxfer-callsite-modes.tsv` showed TEST
   `NONE/NONE` callsites routed to `a/a.n`, obfusjack non-defer `NONE/NONE`
   callsites routed to `a/da.xa`, defer `NONE/NONE` callsites stayed on
   `a/da.wa`, and THREAD callsites stayed on generic helpers. PrintInlining at
   `build/jvm-runtime-perf/p2-2-17-after-full-test-print-inlining.stdout.log`
   showed some hot `a.a::n (97 bytes)` callsites became `inline (hot)`, while
   other hot callsites still hit caller-size limits.

   The fresh ten-run runtime gate at
   `build/jvm-runtime-perf/repeats-p2-2-17-10x/runtime-medians.tsv` rejected
   the change: full TEST `Calc` sorted values were
   `172,175,178,180,180,181,182,183,187,189`, medians `180/181 ms`,
   regressing from accepted P2.2.16 `171/172 ms`. Full obfusjack `Seq` sorted
   values were `300,301,301,302,302,302,303,304,305,309`, medians
   `302/302 ms`, not regressing, but the TEST regression is sufficient to
   reject. Non-target medians were `Platform=73/75 ms`, `Virtual=79/79 ms`,
   `Parallel=7/8 ms`, and `VThreads=8/9 ms`. Stderr files were empty, no fresh
   `hs_err` files were found, and runtime/log scans found no verifier,
   linkage, fallback, skip, or crash markers after excluding the expected
   `Caught MyException: boom`. The timing loop reported one known fixture
   `Test 1.6: Pool FAIL`, matching the already documented accepted-baseline
   flake pattern, while the broad JVM suite had already passed.

   Source changes for P2.2.17 were reverted. Scoped `git diff --` on
   `CffSharedState.java`, `CffClassSetup.java`, `CffMaterialTables.java`, and
   `CffKeyTransferRewriter.java` is empty. Accepted-source regeneration with
   `./gradlew :neko-test:test --tests dev.nekoobfuscator.test.JvmFullObfuscationPerfTest --rerun-tasks`
   passed with `BUILD SUCCESSFUL in 13s` and `19 actionable tasks: 19
   executed`, restoring the build artifacts to the accepted P2.2.16 source
   behavior. P2.2.17 must not be retried as a ticket-mode helper split.
...
### P4 Continue evidence-backed key-update runtime optimization

Status: `[-]` evidence gathered; plan-intake review pending before any runtime
source edit.

Scope:

- Optimize only generic JVM CFF/key-update runtime overhead proven in the
  current full-obfuscation and cff-only runtime artifacts.
- Do not change CFF block boundaries, CFF block selection, fake rows, poison
  rows, material rows, hard-fail rows, dispatch token rows, hidden method keys,
  packed `Object[]` ABI, reflection/MethodHandle compatibility, invokedynamic
  bootstrap/caching, or transform coverage.
- Do not add a fallback, skip path, original-bytecode path, Java helper layer,
  benchmark-specific path, owner/name/descriptor-specific path, or static-key
  replacement for live key flow.
- P4.1 changes only the internal storage shape for the dynamic CFF object-cell
  key-update buckets. P4.2 is conditional follow-up work only if P4.1 does not
  reach P1/P2 and fresh post-P4.1 evidence still points at inline key-update
  decode overhead.

Fresh P4 evidence:

- P3 source-controlled ablation remains failing after the accepted P2.2.16
  source: TEST cff-only-stack `Calc=94 ms`, TEST full `Calc=175 ms`,
  obfusjack cff-only-stack `Seq=290 ms`, and obfusjack full `Seq=301 ms`.
  This proves the remaining P1/P2 gap is still present without native code and
  without requiring invokedynamic as the primary path. The source-controlled
  report is
  `build/test-jvm-runtime-ablation/jvm-runtime-ablation-report.json`.
- Fresh P4 JFR on `TEST-cff-only-stack.jar` records hot samples in obfuscated
  app methods `a.u.l(Object[], long)`, `a.u.m(Object[], long)`,
  `a.u.o(Object[], long)`, and `a.u.n(Object[], long)`. Helper samples include
  token material and transition material helpers, while stderr remains empty.
  Artifacts:
  `build/jvm-runtime-perf/p4-jfr/p4-test-cff-only.jfr`,
  `build/jvm-runtime-perf/p4-jfr/p4-test-cff-only.hot-methods.txt`,
  `build/jvm-runtime-perf/p4-jfr/p4-test-cff-only.execution-samples.txt`,
  `build/jvm-runtime-perf/p4-jfr/p4-test-cff-only.stdout.log`, and
  `build/jvm-runtime-perf/p4-jfr/p4-test-cff-only.stderr.log`.
- Fresh P4 JFR on `obfusjack-cff-only-stack.jar` records hot samples in
  `a.a.y(double[][], int, int, double[][], double[][], long)`,
  `a.a.e(Object[], long)`,
  `a.a.x(double[][], int, double[][], double[][], long, int)`,
  `a.da.ab(long, int, int, int, Object[], int, int, int)`, and
  `a.da.ua(long, int, int, int, Object[], int, int, long[])`.
  Key-transfer helper `a.da.wa` is not a dominant `Seq` sample, so P4 does not
  retry the rejected P2.2.17 ticket-mode split. Artifacts:
  `build/jvm-runtime-perf/p4-jfr/p4-obfusjack-cff-only.jfr`,
  `build/jvm-runtime-perf/p4-jfr/p4-obfusjack-cff-only.hot-methods.txt`,
  `build/jvm-runtime-perf/p4-jfr/p4-obfusjack-cff-only.execution-samples.txt`,
  `build/jvm-runtime-perf/p4-jfr/p4-obfusjack-cff-only.stdout.log`, and
  `build/jvm-runtime-perf/p4-jfr/p4-obfusjack-cff-only.stderr.log`.
- Fresh PrintInlining on the same artifacts shows token material helpers
  `a.a::j (302 bytes)` and `a.da::ta (296 bytes)` are repeatedly called from
  large hot methods and are sometimes too large until C2 hot inlining occurs.
  Class-key helpers are already small and hot-inlined, so P4 does not target
  class-key helper inlining. Artifacts:
  `build/jvm-runtime-perf/p4-print-inlining/p4-test-cff-only.stdout.log`,
  `build/jvm-runtime-perf/p4-print-inlining/p4-test-cff-only.stderr.log`,
  `build/jvm-runtime-perf/p4-print-inlining/p4-obfusjack-cff-only.stdout.log`,
  and
  `build/jvm-runtime-perf/p4-print-inlining/p4-obfusjack-cff-only.stderr.log`.
- Fresh generated bytecode inspection at
  `build/jvm-runtime-perf/p4-bytecode/test-cff-only-a-u-a-a.javap.txt` and
  `build/jvm-runtime-perf/p4-bytecode/obfusjack-cff-only-a-a-a-da-a-ba.javap.txt`
  counted hot cff-only callsites: obfusjack `y` has `ta=30`, `cb=14`,
  `sa=14`; obfusjack `x` has `ta=52`, `cb=24`, `sa=20`, `wa=16`;
  obfusjack `e` has `ta=71`, `cb=31`, `sa=44`, `wa=35`; obfusjack `fa`
  has `ta=34`, `cb=18`, `sa=23`, `ua=45`, `wa=2`. TEST hot methods
  similarly contain repeated token material and key-update helper calls.
- Source evidence in `CffMaterialTables.installClassKeyTableInit` initializes
  the first `CLASS_KEY_TABLE_SIZE` material carrier entries as per-bucket
  `AtomicLong` objects containing the packed dynamic object-cell word.
  Source evidence in `CffMaterialTables.emitAlignedTokenMaterialObjectMask`
  computes the bucket index from live `guard/path/block`, reads
  `material[index]`, casts it to `AtomicLong`, calls `getPlain`, derives the
  same live `currentMask`, `result`, and `nextEpoch`, then calls `setPlain`.
  This is an exact current runtime key-update path for the hot token material
  helpers.
- Source evidence in `CffDispatchEmitter.useDeltaTransitionKeys` also shows
  only non-handler `DIRECT_ISLAND` inline transitions use delta block-key
  updates; `ControlFlowFlatteningLr.chooseEdgeKind` routes many non-handler
  multi-island edges through `HUB` or `ALIAS_HUB`, which still use full
  `emitDecodeBlockKeys`. This is recorded as P4.2-only evidence, not the first
  implementation edit.
- Source audit for object-cell consumers is bounded to generated CFF material
  internals. `rg` finds first-slot object-cell reads/writes in
  `CffMaterialTables.emitAlignedTokenMaterialObjectMask` and
  `CffIslandMaterial.emitTokenMaterialObjectMask`. The apparent
  `CffKeyStateEmitter.emitClassObjectTokenMaskAndUpdate` object-helper path is
  unreachable in the active table path: `emitEncryptedToken` emits
  `table.tokenMaterialHelperName()` and returns when `activeKeyTable != null`,
  then calls `emitClassObjectTokenMaskAndUpdate` only after that return point;
  the callee immediately returns when `activeKeyTable == null`. Fresh generated
  artifact proof also shows
  `rg -n "([Ljava/lang/Object;IIIIIIIII)I" build/jvm-runtime-perf/p4-bytecode`
  has no matches. `CffClassSetup` emits
  `AtomicLong` cells for separate class-integrity carriers, not the material
  carrier's first `CLASS_KEY_TABLE_SIZE` slots. Therefore P4.1 must update both
  active material-carrier consumers and must not touch class-integrity
  carriers.

Rejected-route separation:

- P4.1 does not reuse P2.2.3 packed-pair row caching, P2.2.6 object-cell
  stack-carry, P2.2.13 helper-local step mask caching, or P2.2.14 material
  carrier reference caching. It does not cache decoded token rows, carry a
  result on the operand stack across the update, cache a material carrier
  reference per protected method, or change step-material helpers.
- P4.1 does not reuse P1.4/P1.5 direct-island/no-domain changes, P1.6/P2.2.7
  row-base pre-scaling, P2.2.8 direct `out[]` stores, P2.2.11 transition
  carrier replacement, P2.2.12/P1.7 compressed-island cursor changes, or
  P2.2.17 ticket-mode splitting.
- P4.2, if reached, must not change transition-material helper rows,
  `out[]` layout, direct-island domain decoding, or row-base call-boundary
  contracts. It may only reuse the already-existing live delta block-key
  update formula for eligible non-handler inline transitions.
...
- Add one internal CFF material-carrier slot for the dynamic object-cell
  bucket array and initialize it as an `AtomicLongArray(CLASS_KEY_TABLE_SIZE)`
  containing the exact packed words currently stored in the per-bucket
  `AtomicLong` objects.
- Update both active material-carrier object-cell decode/update paths:
  `CffMaterialTables.emitAlignedTokenMaterialObjectMask` and
  `CffIslandMaterial.emitTokenMaterialObjectMask`. Both paths must load the
  same `AtomicLongArray`, use the same live bucket index, read with
  `AtomicLongArray.getPlain(index)`, derive the same packed-word format,
  `currentMask`, `result`, and `nextEpoch`, and write with
  `AtomicLongArray.setPlain(index, packed)`.
- Keep the carrier field type as `[Ljava/lang/Object;`, keep all existing
  material table slots addressable by constants, and update only CFF-internal
  slot constants and helper bytecode generation.
- Do not change token material row contents, table sizes, encrypted words,
  row alignment, key-transfer helpers, transition helpers, step helpers,
  compressed-island row formulas, class-key words, class-integrity formulas, or
  any public/transformed method ABI.
- Do not replace `AtomicLong.getPlain/setPlain` with primitive `long[]`
  `LALOAD/LASTORE`. P4.1 keeps the same atomic class plain-access API family by
  using `AtomicLongArray.getPlain/setPlain`, which is present in the active JDK
  and avoids the 64-bit primitive array tearing concern raised in the failed
  plan-intake review.
- Update `ControlFlowFlatteningAlgebraicAuditTest.updatesSidecarCell` in the
  same P4.1 implementation commit so the audit accepts the new
...
- This P4 section and the P3/P4 runtime evidence above.
- Fresh bytecode evidence before implementation showing the current hot token
  material helpers and compressed-island helpers contain `AtomicLong`
  `CHECKCAST`, `getPlain`, and `setPlain` on the cff-only/full generated
  artifacts.
- Source audit showing no non-CFF public ABI depends on the concrete runtime
  class or per-bucket identity of the first `CLASS_KEY_TABLE_SIZE` carrier
  entries. The material carrier field remains private/synthetic generated CFF
  state, and runtime object-cell consumers are only the two CFF-generated
  helper paths named above.
- Source audit of class-integrity `AtomicLong` usage must remain unchanged.
  Those carriers are not P4.1 object-cell storage and must not be rewritten.
- Current generated-artifact audit must show the legacy
  `([Ljava/lang/Object;IIIIIIIII)I` object-helper descriptor is absent from
  TEST and obfusjack P4 bytecode dumps before implementation; if a future run
  produces this descriptor, P4.1 must stop and add that active consumer to the
  implementation scope before source edits continue.
- Test audit scope must be updated so `updatesSidecarCell` recognizes
  `java/util/concurrent/atomic/AtomicLongArray` `CHECKCAST` plus
...
| Slot region | Current layout | P4.1 layout |
| --- | --- | --- |
| `0..63` | 64 per-bucket `AtomicLong` object-cell entries | no semantic reader may use these entries; they are removed from object-cell semantics after both active consumers are updated |
| `OBJECT_CELL_ARRAY_SLOT` | absent | one `AtomicLongArray` holding 64 packed dynamic object-cell words |
| `TOKEN_MATERIAL_WORDS_SLOT` and later slots | start at `CLASS_KEY_TABLE_SIZE` | shifted by one internal slot and still addressed only through constants |
| Generated carrier field | `[Ljava/lang/Object;` | unchanged |

Invariant mapping:

| Current object-cell path | P4.1 object-cell path | Preserved invariant |
| --- | --- | --- |
| `installClassKeyTableInit` creates 64 `AtomicLong` cells, one per dynamic bucket | `installClassKeyTableInit` creates one `AtomicLongArray(64)` in an internal carrier slot | there are still 64 dynamic buckets seeded from the same `objectValues[i]` and `cffObjectCellEpoch(clinitMask, i)` |
| token and island helpers compute `index = f(live guard, live path, live block, row words)` | unchanged | bucket choice remains driven by live CFF key state |
| helper reads `AtomicLong.getPlain()` from bucket `index` | helper reads `AtomicLongArray.getPlain(index)` | atomic plain read API semantics and packed word value are preserved |
| helper decodes `epoch`, `currentMask`, `result`, and `nextEpoch` | unchanged formulas | key-update semantics and nonlinear live state dependence are unchanged |
| helper writes packed updated word with `AtomicLong.setPlain()` | helper writes the same packed word with `AtomicLongArray.setPlain(index, packed)` | dynamic bucket state still mutates on every token or island material update |
| generated carrier remains a synthetic `Object[]` field | unchanged field descriptor with one additional internal slot | packed carrier ABI for transformed methods remains unchanged |

P2.2.2 separation:

- P2.2.2 deliberately preserved the 64 separate `AtomicLong` object cells while
  changing a smaller helper path. That preservation was a scope boundary for
  P2.2.2, not evidence that per-bucket Java object identity is a runtime
  correctness invariant.
- P4.1 is only allowed because P4 evidence now identifies object-cell
  key-update storage itself as a hot runtime path and because the source audit
  bounds all semantic consumers to generated CFF material helpers. If review
  finds any semantic consumer of per-bucket object identity, P4.1 must be
...
- Run fresh 10x original/full JVM runtime repeats for TEST and obfusjack,
  because the latest user request requires 10x timing due to variance.
- Run the P3 ablation report again after the implementation so the cff-only
  stack can be compared against the full stack with source-controlled evidence.
- Inspect generated bytecode for TEST and obfusjack hot token and island
  helpers. The changed material object-cell path must no longer invoke
  `java/util/concurrent/atomic/AtomicLong.getPlain()J` or
  `java/util/concurrent/atomic/AtomicLong.setPlain(J)V`, and must instead
  invoke `java/util/concurrent/atomic/AtomicLongArray.getPlain(I)J` and
  `java/util/concurrent/atomic/AtomicLongArray.setPlain(IJ)V`.
- Inspect CFF topology and dry-run counts. `realRows`, `fakeRows`,
  `poisonRows`, `liveDispatchTokenRows`, material rows, helper descriptors
  other than the object-cell storage slot, and missing-row counters must not
  show reduced coverage or new blockers.
- Inspect stdout, stderr, obfuscation logs, repeat logs, and `hs_err` files for
  verifier errors, linkage errors, crashes, fallback, skip behavior,
  plaintext marker leaks, or original-bytecode fallback markers.

Completion criteria:

- Targeted JVM validation passes freshly after the implementation.
...
- Full TEST `Calc` 10x lower/upper median improves from the current accepted
  ...
  improve, reject and revert P4.1 before attempting any later runtime change.
- Non-target obfusjack rows remain inside their current accepted spread or the
  plan's 10% regression budget, whichever is stricter for the row.
- P3 ablation output remains valid for every row, with empty stderr logs and no
  forbidden runtime markers.
- Bytecode inspection proves the hot token material helper moved from
  per-bucket `AtomicLong` cell updates to `AtomicLongArray` indexed updates,
  and that compressed-island material uses the same object-cell storage.
- Subagent implementation review passes before commit.
...
- Change only inline `CffDispatchEmitter.emitTransitionCore` key updates by
  extending the already-existing `emitDeltaBlockKeys` form from
  `DIRECT_ISLAND` to `HUB` and `ALIAS_HUB` when the edge role is a real
  non-handler control-flow edge: `FALLTHROUGH`, `GOTO`, `CONDITIONAL_TRUE`,
  `CONDITIONAL_FALSE`, `SWITCH_CASE`, or `SWITCH_DEFAULT`.
- Keep `HANDLER`, `FAKE`, and `POISON` on the current decode path for
  `HUB`/`ALIAS_HUB`. Preserve the existing direct-island behavior, including
  existing direct-island `FAKE`/`POISON` delta updates.
- Preserve domain-token stores for `HUB` and `ALIAS_HUB`; do not remove,
  bypass, delay, or weaken domain materialization.
- Do not change transition-material helper rows, transition outliner calls,
  transition row layout, `out[]` layout, direct-island domain decode, CFF block
  boundaries, CFF block selection, fake rows, poison rows, handler bridges, or
  transform coverage.
- The implementation may change only `useDeltaTransitionKeys` and a local
  predicate/helper needed to express eligible real non-handler roles.

Fresh P4.2 evidence:

- P4.1 was rejected and fully reverted. Its 10x evidence at
  `build/jvm-runtime-perf/p4-atomiclongarray-10x/runtime-medians.tsv` showed
  TEST full `Calc=186/186 ms` and obfusjack full `Seq=305/305 ms`, so object
  cell storage is not the next accepted route.
- P4 JFR shows hot samples are dominated by transformed application methods
  that contain inline CFF code, not only transition-material helper rows:
  `build/jvm-runtime-perf/p4-jfr/p4-obfusjack-cff-only.hot-methods.txt`
  records obfusjack `a.a.y`, `a.a.e`, and `a.a.x` at `731+559+492` samples,
  while `a.da.ua` transition-material helper has `202` samples. Bytecode
  inspection in
  `build/jvm-runtime-perf/p4-bytecode/obfusjack-cff-only-a-a-a-da-a-ba.javap.txt`
  shows obfusjack hot methods `y`, `x`, and `e` have `0` calls to
  `a.da.ua`, so their CFF transition-key work is inline rather than
  transition-material helper work. The same inspection shows `fa` has `45`
  transition-material helper calls, so P4.2 does not claim to optimize that
  separate helper path.
- P4 JFR for TEST at
  `build/jvm-runtime-perf/p4-jfr/p4-test-cff-only.hot-methods.txt` records
  `a.u.l`, `a.u.m`, and `a.u.o` as the dominant hot application methods.
  Bytecode inspection in
  `build/jvm-runtime-perf/p4-bytecode/test-cff-only-a-u-a-a.javap.txt` shows
  these three hot methods have `0` calls to transition-material helper `a.a.k`;
  only `a.u.n` has such calls. Therefore P4.2 targets the inline key-update
  path actually present in the dominant TEST hot methods.
- Fresh generated bytecode inspection of the same TEST hot methods counted the
  exact current `emitDecodeBlockKeys` shape P4.2 would replace: three decoded
  temporary words are committed to `guard/path/block`, followed by pc,
  method-key, domain-token stores and a `goto`. The counts are
  `a.u.l=7`, `a.u.m=5`, and `a.u.o=4` non-direct
  `decode_commit_domain_goto` groups. These groups are not transition-material
  helper calls, because the same method slices have `0` `a.a.k` calls.
- Fresh generated bytecode inspection also bounds P4.2's expected P1 impact:
  obfusjack hot `a.a.y` and `a.a.x` have `0`
  `decode_commit_domain_goto` groups, while `a.a.e` has `3`. Therefore P4.2
  is not claimed as the primary obfusjack P1 repair. It is allowed only as a
  generic inline transition-key update improvement for the TEST/P2 dominant
  path, with the 10x gate rejecting it if obfusjack `Seq` regresses or if TEST
  does not improve.
- Source evidence in `ControlFlowFlatteningLr.chooseEdgeKind` routes real
  non-handler transitions to `HUB`/`ALIAS_HUB` for switch choices `2`, `7`,
  and the default `3`, `4`, `6` cases when multiple islands exist. Source
  evidence in `CffDispatchEmitter.useDeltaTransitionKeys` currently allows
  delta updates only for `DIRECT_ISLAND` and non-handler roles, so eligible
  real `HUB`/`ALIAS_HUB` inline transitions still use `emitDecodeBlockKeys`.
- Source evidence in `CffDispatchEmitter.emitTransitionCore` computes
  `keyBaseLocal` by calling either `emitDeltaBlockKeys` or
  `emitDecodeBlockKeys`, then uses the same `keyBaseLocal` to store pc,
  method-key, and, for non-direct edges, domain tokens. Therefore extending
  delta updates to real `HUB`/`ALIAS_HUB` edges preserves later domain-token
  materialization as long as `keyBaseLocal` is still produced from the same
  `transitionBaseSeed`.

Source invariant proof for eligible real non-handler transitions:

- `ControlFlowFlatteningPass` allocates `pcLocal`, `guardLocal`,
  `pathKeyLocal`, `blockKeyLocal`, and `domainLocal` starting at the original
  `mn.maxLocals`, then increases `mn.maxLocals` for generated CFF state. The
  original application bytecode cannot legally target these appended locals;
  generated CFF code is the only writer after the method is rewritten.
- `insertIslandDispatchers` initializes the entry block's `guard/path/block`
  locals from the entry block key state and stores the entry pc, method key,
  and domain token before entering the dispatcher. Dispatcher selection then
  jumps into a block whose current CFF locals represent that block's
  `CffBlockKeyState`.
- `rewriteBlockExit` passes `requireBlockKey(block.label(), ...)` as
  `sourceKeys` for `GOTO`, `CONDITIONAL_TRUE`, `CONDITIONAL_FALSE`, and
  `FALLTHROUGH` transitions. The inserted transition is placed at the original
  block terminator or immediately after the last real instruction, before the
  rewritten jump leaves the block. Therefore the live `guard/path/block` locals
  are the current block's source key state at the transition point.
- `rewriteLookupSwitch` and `rewriteTableSwitch` rewrite the original switch
  labels to generated `setLabels`, keep the original source label as
  `source`, and add tail transitions for `SWITCH_DEFAULT` and `SWITCH_CASE`
  that pass `requireBlockKey(source, ...)` as `sourceKeys`. The switch consumes
  its selector and branches to a generated tail without writing CFF locals, so
  the same source key-state invariant holds for switch tails.
- P4.2 excludes `HANDLER`, `FAKE`, and `POISON` because those paths can enter
  from exceptional, synthetic, or pollution control flow where the above real
  block-exit source-key invariant is not the acceptance proof. It also leaves
  transition outliner/materialized helper calls unchanged.

Invariant mapping:

| Current real HUB/ALIAS inline transition | P4.2 transition | Preserved invariant |
| --- | --- | --- |
| `emitDecodeBlockKeys` computes `keyBaseLocal` from live method key and live `guard/path/block` plus `sourceKeys` | `emitDeltaBlockKeys` computes the same `keyBaseLocal` from the same live inputs and `sourceKeys` | live method-entry key and current CFF state still drive every downstream update |
| decoded guard/path/block words are encrypted against `compactControlTokenBase(sourceKeys, transitionBaseSeed)` | delta words are encrypted against the same compact base and are added to the current live locals | if live locals equal `sourceKeys`, the resulting locals equal `targetKeys`; if they do not, the wrong live state still corrupts the transfer |
| `emitStoreTransitionBaseToken` stores target pc from `keyBaseLocal` | unchanged | dispatcher pc token remains protected by the same live base |
| `emitStoreMethodKeyFromBase` stores target method key from `keyBaseLocal` | unchanged | hidden method key propagation remains tied to live transition state |
| non-direct edges store target domain token after key update | unchanged | `HUB`/`ALIAS_HUB` domain routing is preserved |
| handlers, fake rows, poison rows, transition-material helper rows, and outliner calls keep current decode/material behavior | unchanged | exceptional/noise/material surfaces are not folded into this optimization |

Completion criteria:

- Same targeted JVM validation and 10x runtime gate as P4.1.
- Generated bytecode or source diff review proves only eligible real
  non-handler `HUB`/`ALIAS_HUB` inline transitions moved to the delta
  block-key update form. Transition-material helper rows, transition outliner
  calls, direct-island domain decode, and `out[]` layout remain unchanged.
- Full TEST `Calc` improves from the current accepted baseline `177/178 ms`.
  Full obfusjack `Seq` must not regress from `303/304 ms`; if it also
  improves, record that as secondary progress, but do not claim P4.2 completes
  P1. If TEST does not improve or obfusjack regresses, P4.2 is rejected and
  reverted before any later runtime change.
- Subagent implementation review passes before commit.

P4.2 first plan-intake review:

- The first P4.2 plan-intake review failed before any source edit. The review
  found two blockers: the plan did not yet prove that live `guard/path/block`
  locals equal `sourceKeys` at every eligible `FALLTHROUGH`, `GOTO`,
  `CONDITIONAL_*`, and `SWITCH_*` transition, and it did not quantify the
  current generated hot-code decode pattern that P4.2 would replace.
- This revision adds the source invariant proof from local allocation through
...
- Generic generated-helper topology change in the shared CFF material-table
  helpers. No benchmark, class, method, descriptor, owner, or fixture
  special-casing.
- Keep the current token-material row layout, token-material row word count,
  object-cell storage shape, dynamic object-cell epoch update, CFF block
  coverage, CFF block boundaries, hidden key propagation, and transform
  coverage unchanged.
- Do not add fallback behavior, static key recomputation, helper-layer
  compatibility bypasses, Java-side runtime helpers, original-bytecode paths,
  or reduced-obfuscation paths.
- P4.3 is not a retry of rejected token/object storage routes. It does not add
  a token row-pair cache, stack-carry/reordered token update, material carrier
  reference cache, `AtomicLongArray` or primitive object-cell storage,
  object-cell storage replacement, formula change, row-word change, or
  generated callsite reordering. It is helper-boundary splitting only.

Fresh accepted-source evidence:

- Fresh full-JVM-obfuscation artifacts were regenerated by
  `JvmFullObfuscationPerfTest` into `build/test-jvm-full-obf-perf/` after the
  P4.2 revert.
- The fresh 10x accepted-source runtime gate at
  `build/jvm-runtime-perf/p5-accepted-source-10x/runtime-medians.tsv` records
  full TEST `Calc=186/186 ms` and full obfusjack `Seq=309/312 ms`. These are
  the acceptance baseline for P4.3.
- The same 10x gate records full obfusjack `Platform=78/78 ms`,
  `Virtual=86/86 ms`, `Parallel=9/9 ms`, and `VThreads=8/8 ms`.
- All accepted-source stderr logs under
  `build/jvm-runtime-perf/p5-accepted-source-10x/` are empty. Runtime output
  scans found only expected fixture text (`Test 2.8: Sec ERROR` and
  `Caught MyException: boom`) and no verifier, linkage, fallback, skip, crash,
  `FAIL`, `translated=0`, or native-compilation markers.

Fresh hot-path evidence:

- JFR at `build/jvm-runtime-perf/p5-jfr/` records obfusjack full runtime hot
  samples dominated by transformed application methods: `a.a.y` at `136`
  samples (`32.69%`), `a.a.x` at `114` samples (`27.40%`), and `a.a.e` at
  `72` samples (`17.31%`). The same JFR records a diagnostic-run obfusjack
  `Seq=330 ms`.
- Bytecode slices in `build/jvm-runtime-perf/p5-bytecode/` show the dominant
  obfusjack sequential methods `a.a.x` and `a.a.y` contain no invokedynamic
  callsites, no shared island helper calls, and no transition-material helper
  calls. Their hot generated work is inline CFF/key-update code.
- The same bytecode slices count token/key helper calls in hot methods:
  `a.a.e` has `68` calls to token-material helper `a.da.ta`, `44` calls to
  class-key helper `a.da.sa`, and `31` calls to method-key helper `a.da.cb`;
  `a.a.x` has `42` token-material calls, `16` class-key calls, and `19`
  method-key calls; `a.a.y` has `30` token-material calls, `14` class-key
  calls, and `14` method-key calls.
- Focused PrintInlining output at
  `build/jvm-runtime-perf/p5-print-inlining-focused/obfusjack-focused.filtered.log`
  records `a.da::ta (296 bytes)` repeatedly rejected in hot `a.a.x` and
  `a.a.y` inline contexts with `callee is too large`. It also records
  `a.da::cb (43 bytes)` rejected in several hot contexts, while
  `a.da::sa (29 bytes)` is inlined.
- Source evidence in `CffKeyStateEmitter.emitEncryptedToken` shows active class
  tables route every encrypted token decode through the shared token-material
  helper. Source evidence in
  `CffMaterialTables.installEncryptedTokenMaterialHelper` shows that the
  current helper combines class mask material, object-cell mask/update, and
  control mask material in one helper body. The object-cell portion includes
  `AtomicLong.getPlain`, epoch/mask decoding, `AtomicLong.setPlain`, and row
  word driven mixing.

Planned repair:

- Add one generated shared helper for the object-cell token-material mask/update
  path and move the existing object-cell bytecode from the token-material helper
  into that helper without changing the math, row words, material carrier, or
  object-cell storage type.
- Generate that helper in `CffClassSetup.ensureSharedClassHelpers` with the
  unique-name family `__neko_cff_tobj$...`, descriptor
  `([Ljava/lang/Object;[JIIII)I`, and access
  `ACC_PUBLIC | ACC_STATIC | ACC_SYNTHETIC`. The owner is the same shared helper
  owner currently used for `__neko_cff_tmat$...` (`clazz.name()`), and the
  interface-owner flag is `clazz.isInterface()`.
- Install the object-cell helper before installing `__neko_cff_tmat$...`.
  Extend the `installEncryptedTokenMaterialHelper` plumbing in `CffSharedState`
  and `CffMaterialTables` to receive the object-cell helper owner, name, and
  interface flag. The new helper does not need to be exposed through
  `CffSharedClassHelpers` because no generated application callsite may invoke
  it directly; only the generated token-material helper invokes it.
- Keep `__neko_cff_tmat$` as the caller-visible token-material helper used by
  `CffKeyStateEmitter.emitEncryptedToken`, but reduce its body to row load,
  class-mask material, call to the object-cell helper, control-mask material,
  and return. The caller-visible helper must remain driven by live
  `guard/path/block` inputs and material row words.
- The caller-visible token-material helper descriptor remains exactly
  `([Ljava/lang/Object;IIII)I`; `CffKeyStateEmitter.emitEncryptedToken` and
  existing generated token-material callsites are not changed.
- The token-material helper invokes the object-cell helper with `INVOKESTATIC`
  and the shared-helper interface flag. The invocation passes only the material
  carrier, decoded token row array, token row long-base, and live
  `guard/path/block` values.
- The new object-cell helper must receive the material carrier, material row,
  row base, and live `guard/path/block` inputs, read the same fixed object-mask
  row words currently read by `emitAlignedTokenMaterialObjectMask`, update the
  same `AtomicLong` cell with the same `getPlain`/`setPlain` behavior, and
  return the same object mask word currently xor-mixed by the token-material
  helper.
- The implementation may change only the helper-installation topology and the
  code-generation call plumbing needed to pass the object-cell helper owner,
  name, and descriptor into the token-material helper installer.
- `CffIslandMaterial.emitTokenMaterialObjectMask` and the compressed/island
  material helper path are out of scope for P4.3. They keep their current inline
  object-cell update bytecode. P4.3 validation must inspect generated bytecode
  to prove the compressed/island helper still owns that path and was not
  redirected to the new token-object helper.

Required validation target:

- Run the targeted JVM validation suite used by P4.2 with repository
...
- Run a fresh 10x runtime gate for original TEST, full TEST, original
  obfusjack, and full obfusjack, writing results under a new
  `build/jvm-runtime-perf/p4-token-helper-split-10x/` directory.
- Run focused PrintInlining for `a.a::x` and `a.a::y` against the fresh full
  obfusjack jar and write output under
  `build/jvm-runtime-perf/p4-token-helper-split-print-inlining/`.
- Inspect generated bytecode/source topology to prove token-material row layout,
  row word count, CFF block coverage, hidden key propagation callsites, and the
  object-cell update semantics were not reduced.
- Inspect generated bytecode to prove `__neko_cff_tmat$...` still has descriptor
  `([Ljava/lang/Object;IIII)I`, invokes exactly the new `__neko_cff_tobj$...`
  descriptor with `INVOKESTATIC`, and remains the only helper called by
  generated token-material callsites.
- Inspect generated bytecode to prove the compressed/island material helper path
  still contains its existing object-cell `AtomicLong.getPlain`/`setPlain`
  sequence and is not redirected to `__neko_cff_tobj$...`.
- Inspect stdout, stderr, runtime markers, and fresh `hs_err` files.

Completion criteria:

- Targeted JVM validation passes.
- Fresh bytecode topology proves the change is a helper split only: no token row
  layout change, no object-cell storage shape change, no CFF block coverage
  change, no block-boundary change, no fallback, no static key path, and no
  removed dynamic object-cell update.
- Fresh bytecode topology proves rejected routes remain absent: no row-pair
  cache, no stack-carry or token callsite reordering, no material carrier cache,
  no `AtomicLongArray`/primitive object storage replacement, and no token
  formula or row-word change.
- Fresh bytecode topology proves the public token-material helper ABI remains
  `([Ljava/lang/Object;IIII)I`; the new object-cell helper ABI is exactly
  `([Ljava/lang/Object;[JIIII)I`; and the compressed/island material helper
  path remains unchanged.
- Focused PrintInlining proves the caller-visible token-material helper is no
  longer rejected as `callee is too large` in the hot `a.a.x` and `a.a.y`
  contexts. If it remains too large, P4.3 is rejected.
- Fresh 10x medians improve full obfusjack `Seq` from the accepted-source
  baseline `309/312 ms` and do not regress full TEST `Calc` from `186/186 ms`.
  If obfusjack does not improve or TEST regresses, P4.3 is rejected and
  reverted before any later runtime change.
- All stderr/log/marker/hs_err inspections are clean.
- Subagent implementation review passes before commit.

P4.3 first plan-intake review:

- The first P4.3 plan-intake review failed before any source edit. The review
  found three blockers: the new helper ABI/topology was underspecified, the
  relationship to `CffIslandMaterial.emitTokenMaterialObjectMask` was
  ambiguous, and the plan did not explicitly separate P4.3 from previously
  rejected token/object routes.
- This revision records the exact object helper descriptor
  `([Ljava/lang/Object;[JIIII)I`, generated-name family
  `__neko_cff_tobj$...`, access flags, owner/interface flag, install order,
  invocation opcode, unchanged public token helper descriptor, and metadata
  plumbing. It also states that `CffIslandMaterial.emitTokenMaterialObjectMask`
  remains out of scope and requires bytecode proof that the compressed/island
  path is unchanged. Finally, it records the rejected-route separation.
- Re-review passed before source edits. The reviewer confirmed that the revised
  plan has concrete evidence, generic scope, preserved invariants, bounded
