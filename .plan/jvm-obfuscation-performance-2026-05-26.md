...
Status: `[ ]` parked evidence note only; this plan-only commit does not approve
P4.2 implementation. Do not implement P4.2 until P4.1 is completed or rejected
with fresh runtime evidence, this section is rewritten with exact semantic
proof, and a new subagent plan-intake review passes.

Scope:

- If P4.1 does not reach P1/P2, and fresh post-P4.1 evidence still shows
  inline transition key updates as a hot runtime path, extend the existing
  `emitDeltaBlockKeys` form from non-handler `DIRECT_ISLAND` edges to eligible
  non-handler `HUB` and `ALIAS_HUB` inline transitions.
- Keep handler edges on the current decode path unless new evidence and a new
  plan section prove handler-delta safety.
- Preserve domain-token stores for `HUB` and `ALIAS_HUB`; do not remove or
  bypass domain materialization.
- The future P4.2 plan must prove, per edge family, that the live
  `guard/path/block` state at the candidate update is the same state consumed
  by the current decode path. It must explicitly exclude or separately prove
  `FAKE`, `POISON`, exceptional, and handler-related paths. Until that proof is
  recorded, P4.2 remains non-implementable.

Required evidence before editing P4.2:

- Fresh post-P4.1 10x runtime results and JFR/PrintInlining attribution.
- Generated bytecode or transform-report evidence proving hot remaining
  transition key updates are inline `emitTransitionCore` paths, not
  transition-material helper rows already handled by P2.2.10 or direct-island
  domain changes rejected by P1.4/P1.5.
- Subagent plan-intake review specific to the post-P4.1 P4.2 evidence.

Completion criteria:

- Same targeted JVM validation and 10x runtime gate as P4.1.
- Generated bytecode proves only eligible non-handler inline transitions moved
  to the delta block-key update form; transition-material helper rows and
  `out[]` layout remain unchanged.
- Full TEST `Calc` and full obfusjack `Seq` both improve from the fresh
  post-P4.1 baseline, or P4.2 is rejected and reverted.
- Subagent implementation review passes before commit.
